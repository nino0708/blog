...
## X投稿文の文字数ルール（定期実行「編集部」の手順7に適用。指示文の「各120字前後」より優先する）

X の上限は280カウントで、数え方が言語で違う。英語を「120字前後」で書くとURL込みで上限を超える（実例: 2026-09-25 の英語投稿が286カウントで投稿できなかった）。

- 数え方: 日本語（かな・漢字・全角記号）と絵文字は1文字=2、半角英数字・空白・改行は1文字=1、URLは長さに関係なく1本=23。
- 目安: 日本語版は本文（URL以外）100文字前後、英語版は本文220文字（スペース込み）以内。
